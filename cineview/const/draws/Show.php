          ...
          <video controls playsinline poster="../../uploads/thumbnail/<?php echo $thumbnail; ?>" id="player">
            ...
            if ($GLOBALS['streaming'] == "1") {
              try {
                ...
                foreach ($result as $row) {
            ?>
                  <source src="<?php echo $row[2]; ?>" type="video/mp4" size="<?php echo $row[3]; ?>">
                <?php
                }
            ...
            if ($logged == "1") {
              if ($role == "user") {
                ...
                try {
                  ...
                  $result = $stmt->fetchAll();
                  if (count($result) < 1) {
            ?><b id="wait"><button id="<?php echo $item_id; ?>" onclick="add_fav(this.id);" class="article__favorites" type="button"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
