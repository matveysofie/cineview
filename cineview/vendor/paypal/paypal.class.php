...
class MyPayPal
{
	...
	{
		// Настройка свои учетные данные API, конечную точку PayPal и версию API.
		...
		// Отключите сервер и одноранговую проверку (концепция TrustManager).
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, FALSE);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch, CURLOPT_POST, 1);
		// Задайте операцию API, версию и подпись API в запросе.
		$nvpreq = "METHOD=$methodName_&VERSION=$version&PWD=$API_Password&USER=$API_UserName&SIGNATURE=$API_Signature$nvpStr_";
		// Установите запрос в качестве ПОЛЯ ЗАПИСИ для curl.
